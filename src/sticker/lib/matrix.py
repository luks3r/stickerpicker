...
class MediaInfo(TypedDict, total=False):
    ...
    thumbnail_info: NotRequired[MediaInfo]


StickerInfo = TypedDict(
    "StickerInfo",
    {
        "id": str,
        "body": str,
        ...
        "info": MediaInfo,
        "msgtype": str,
        "net.maunium.telegram.sticker": NotRequired[dict],
    },
)
...
async def load_config(path: str) -> None:
    ...
    try:
        ...
        whoami_url = (
            URL(homeserver_url) / "_matrix" / "client" / "r0" / "account" / "whoami"
        )
    ...
    if not homeserver_url:
        raise ValueError("Homeserver URL not set")
...
async def upload(data: bytes, mimetype: str, filename: str) -> str:
    if not access_token or not upload_url:
        raise ValueError("Matrix config not loaded")
    ...
    async with (
        ...
        sess.post(url, data=data, headers=headers) as resp,
    ):
